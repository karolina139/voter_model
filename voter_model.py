...
def model(M,LL,q,p,f,option, model_type, klatki):
		...
		modulo = klatki
		values = np.ones([M+1])
		values[0] = np.sum(np.copy(LL)) /Nn

		placeholder = st.empty()
		ploting(values[0],np.copy(LL), -1,placeholder)
		...
		for k in range(M):
				...
				values[k+1] = np.sum(np.copy(LL)) /Nn
				if k%modulo ==0 :
						ploting(values,np.copy(LL), k+1,placeholder)
...
def ploting(values,LL,k, placeholder):
		...
		if k==-1:
			...
			fig2.update_layout(title = 'State of the system, MCS = {}'.format(k), margin=dict(l=10, r=0, t=50, b=0), font=dict(size=20))
