...
def gui():
	...
	start_grid = ['order up', 'order down', 'mess' , 'vertical stripes', 'horizontal stripes', 'circle', 'square', 'chessboard', 'diagonal stripes']
	...
	if select_event == 'conformity + independence':
		...
		MCSA = st.sidebar.slider("Choose MCS", 100,10000)
		...
		MCSB = st.sidebar.slider("Choose MCS", 100,10000)
	...
	if button_start:
		if select_event == 'conformity + independence':
			...
			st.write(model(MCSA,LLA,qA,pA,fA,random_qA,  'model_a'))
		...
		if select_event == 'conformity + nonconformism':
			...
			st.write(model(MCSB,LLB,qB,pB, 0 ,random_qB,  'model_b'))
			#except:
			#	st.write("Choose right parameters")
